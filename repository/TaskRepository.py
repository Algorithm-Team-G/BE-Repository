...
class TaskRepository:
    ...
    def selectAssignedTasks(self, workerId:int) -> dict:
        sql = f"""
            select
                tk.{Task.C_TASK_ID} id,
                tk.{Task.C_TASK_NAME} title,
                tk.{Task.C_START_DATE} begin,
                tk.{Task.C_END_DATE} end
            from {Task.T_NAME} tk
            where tk.{Task.C_WORKER_ID} = {workerId}
    ...
    def selectUnassignedTasks(self) -> dict:
        sql = f"""
            select tk.{Task.C_TASK_ID}, tk.{Task.C_TASK_NAME}, tm.{Team.C_TEAM_NAME}
            from {Task.T_NAME} tk
